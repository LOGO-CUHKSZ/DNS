...
def load_data(del_time: bool, fold: int, num_folds: int = 10, batch_size: int = 128, seed: int = SEED):
    def feat(loc: np.ndarray, vel: np.ndarray) -> np.ndarray:
        return np.concatenate([loc.reshape(*loc.shape[:2], -1), vel.reshape(*vel.shape[:2], -1)], axis=-1)
    ...
    t_length = 49
    time_train = np.stack([np.arange(t_length) for _ in range(len(loc_train))])
    time_test = np.stack([np.arange(t_length) for _ in range(len(loc_test))])
